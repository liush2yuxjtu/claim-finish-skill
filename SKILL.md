...
运行 8 个 checkpoint，依次核查每一项交付物。每项记录 ✅/⚠️/❌ 三级状态。全部完成后生成 HTML 验收报告。
...
**触发方式：** `/claim-finish --fix`

**作用：** 当存在任意 ❌ 阻塞项时，自动派发修复团队（draft agent team）针对每个阻塞 CP 并行修复。

**执行流程：**

1. **收集阻塞清单** — 汇总所有 `❌` 的 CP 项及其 `notes`（阻塞原因）
2. **生成修复 prompt** — 每个 ❌ CP 拼装独立修复 prompt，内容包括：
   - CP 编号、名称、当前状态
   - 阻塞原因（notes 原文）
   - 该 CP 的验收标准（来自 checkpoint 流程描述）
   - 项目目录结构快照（供修复 agent 参考）
3. **派发并行修复 agents** — 每个 ❌ CP 分配一个独立 agent，并行执行修复
4. **收集修复结果** — 各 agent 完成后，主代理汇总结果并追加到 `REPORT_DATA`
5. **重新运行对应 CP** — 修复完成后重新检查对应 checkpoint，更新状态
6. **重新判定** — 根据更新后状态决定后续路径（LGTM / 条件通过 / 再次阻塞）

**修复 agent 的输出要求：**
- 每个 agent 写一个结构化修复报告到 `/tmp/cp<N>-fix-report.md`
- 报告内容：修复动作列表、修改的文件、验证命令及结果
- 主代理读取这些报告注入到 `REPORT_DATA.cp<N>.fixReport`

**注意事项：**
- `--fix` 不会绕过阻塞，只是自动派发修复，不手动阻塞后续
- 如果修复 agent 无法在合理次数内解决，标记为 `⚠️` 并附带说明
- `--fix` 适用于有实际代码/配置可改的场景；对纯粹”文件不存在”类问题尤其有效
...
| 存在任意 ❌，无 --fix | **阻塞发布**，列出必须修复的条目清单，不得宣告完成 |
| 存在任意 ❌，有 --fix | 派发修复团队，修复完成后重新判定 |
